...
class MazeConfig(BaseModel):
    ...
    def validations(self):
        ...
        if not (0 <= ex < w and 0 <= ey < h):
            raise ValidationError("Entry must be inside maze bounds")
        if not (0 <= xx < w and 0 <= xy < h):
            raise ValidationError("Exit must be inside maze bounds")
        if (ex, ey) == (xx, xy):
            raise ValidationError("Entry and exit cannot be in same spot")
        return self
